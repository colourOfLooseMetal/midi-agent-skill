...
// Aliases for common instrument names
 ...
 * @param name - Instrument name (can be alias or full name)
 * @returns Program number (0-127) or 0 (piano) if not found
 */
export function resolveInstrument(name: string): number {
  ...
  for (const [key, value] of Object.entries(GM_INSTRUMENTS)) {
    if (key.includes(normalized) || normalized.includes(key.replace(/-/g, ""))) {
      return value;
    }
  }
...
export function getInstrumentName(programNumber: number): string {
  for (const [name, num] of Object.entries(GM_INSTRUMENTS)) {
 ...
 */
export function getInstrumentsByCategory(category: string): string[] {
  ...
  return Object.entries(GM_INSTRUMENTS)
    .filter(([_, num]) => num >= range[0] && num <= range[1])
    .map(([name]) => name);
}
